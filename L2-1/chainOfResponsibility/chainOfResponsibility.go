...
Шаблон Цепочка выстраивает конвейер обработчиков для поступающих запросов.
Объект «обработчик» выполняет свою часть процессинга и передаёт запрос дальше по цепочке.
Обработчики не влияют друг на друга и не меняют состояние друг друга. Поэтому их легко писать,
отлаживать и переносить между проектами.
...
Яркий пример применения шаблона в Go — цепочка middleware-обработчиков http.Request,
которую можно реализовать самостоятельно или взять из сторонних HTTP-фреймворков.
